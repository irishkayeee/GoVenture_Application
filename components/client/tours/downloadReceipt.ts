...
export async function downloadReceiptPdf(filename: string, html: string): Promise<DownloadReceiptResult> {
  ...
  if (Platform.OS === 'android') {
    return downloadOnAndroid(baseName, base64 ?? '', uri);
  }
  return downloadOnIos(baseName, uri);
}

async function downloadOnAndroid(baseName: string, base64: string, sourceUri: string): Promise<DownloadReceiptResult> {
  ...
  try {
    ...
    if (!dirUri) {
      ...
      if (!permission.granted) {
        return shareFallback(sourceUri, baseName);
      }
...
async function downloadOnIos(baseName: string, sourceUri: string): Promise<DownloadReceiptResult> {
  const dest = new File(Paths.document, `${baseName}.pdf`);
  ...
  if (available) {
    await Sharing.shareAsync(dest.uri, { mimeType: 'application/pdf', dialogTitle: `${baseName}.pdf` });
    return { ok: true, silent: false };
  }
  return { ok: true, silent: true };
}

async function shareFallback(sourceUri: string, baseName: string): Promise<DownloadReceiptResult> {
  const available = await Sharing.isAvailableAsync();
  if (!available) {
    return { ok: false, message: 'No folder was selected, and sharing is unavailable on this device.' };
  }
  await Sharing.shareAsync(sourceUri, { mimeType: 'application/pdf', dialogTitle: `${baseName}.pdf` });
  return { ok: true, silent: false };
}
