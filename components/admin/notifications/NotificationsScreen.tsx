 ...
 * Admin Notifications tab — real feed backed by the `notifications` table.
 * Filter by category/unread, tap a row to mark it read and jump to the
 * related tab, mark-all-read, and per-row delete. Solid-color icon chips
 * only — no emoji, to match the admin panel's professional visual language.
 */
...
export type NotificationCategory = 'booking' | 'payment' | 'document' | 'message' | 'security' | 'reminder' | 'system';
...
const CheckIcon = ({ color }: { color: string }) => (
  <Svg width={13} height={13} viewBox="0 0 24 24" fill="none"><Path d="M20 6L9 17l-5-5" stroke={color} strokeWidth={2.4} strokeLinecap="round" strokeLinejoin="round" /></Svg>
);
...
const CATEGORY_META: Record<NotificationCategory, { Icon: React.FC<{ color: string }>; bg: string; color: string }> = {
  booking:  { Icon: CalendarIcon, bg: '#FFF3E8', color: '#C46B1A' },
  ...
  system:   { Icon: GearIcon,     bg: '#F1F1F1', color: '#6B3318' },
};

const CATEGORY_TAB: Record<NotificationCategory, string> = {
  booking: 'bookings', payment: 'payments', document: 'accounts', message: 'messages',
  security: 'settings', reminder: 'dashboard', system: 'dashboard',
};
...
export default function NotificationsScreen({ recipientId, onNavigateTab }: Props) {
  const { C } = useAppTheme();
  const ns = useMemo(() => makeStyles(C), [C]);
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setError('');
    const params = new URLSearchParams({ recipientType: 'admin', recipientId, limit: '50' });
  ...
  const handlePress = (n: AdminNotification) => {
    ...
    if (!n.isRead) {
      ...
      fetch(NOTIFICATION_MARK_READ_API_URL, {
        ...
        body: JSON.stringify({ recipientType: 'admin', recipientId, notificationId: n.id }),
      }).catch(() => {});
    }
    onNavigateTab(CATEGORY_TAB[n.category]);
  };
  ...
  const handleMarkAllRead = () => {
    ...
    fetch(NOTIFICATIONS_MARK_ALL_READ_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recipientType: 'admin', recipientId }),
    }).catch(() => {});
  };
  ...
  const handleDelete = (id: string) => {
    ...
    fetch(NOTIFICATION_DELETE_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recipientType: 'admin', recipientId, notificationId: id }),
    }).catch(() => {});
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 8 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={ns.filterRow} contentContainerStyle={{ gap: 8, paddingHorizontal: 16 }}>
          {FILTERS.map((f) => {
          ...
          <TouchableOpacity style={[ns.filterChip, unreadOnly && ns.filterChipActive]} activeOpacity={0.8} onPress={() => setUnreadOnly((v) => !v)}>
            <Text style={[ns.filterChipText, unreadOnly && ns.filterChipTextActive]}>Unread only</Text>
          </TouchableOpacity>
        </ScrollView>
